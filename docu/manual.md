...
## Method outline
[//]: -------------------------------
We can estimate summary statistics of new, untyped variants at the same locus, by combining summary statistics for a set of variants and the fine-scale LD structure in the same region. We can formally write this using the conditional expectation of a multivariate normal distribution. 
...
### Imputation quality
We use an adjusted imputation quality that corrects for the effective number of tag SNVs *p.eff* (n=number of individuals in the reference panel). In this case `--lambda` is set to `1e-6`.
